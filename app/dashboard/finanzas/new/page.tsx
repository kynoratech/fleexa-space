...
export default function NewFinancePage() {
  ...
  // No user state needed, workspace is required
  ...
  const save = async (e: any) => {
    e.preventDefault();
    if (!workspace?.workspaceId) return alert("No workspace seleccionado");
    ...
    try {
      await addDoc(collection(db, "workspaces", workspace.workspaceId, "finanzas"), {
        nombre: nombre.trim(),
  ...
  // Removed user check as no user variable is defined
