...
export async function inviteMember(params: InviteMemberParams) {
  ...
  const workspaceSnap = await getDoc(doc(db, "workspaces", params.workspaceId));
  const workspaceName = workspaceSnap.exists() ? workspaceSnap.data().name : "Unknown";
...
export async function acceptInvite(params: AcceptInviteParams) {
  ...
  // 1️⃣ Crear membership (esto SÍ está permitido por rules)
  await setDoc(
  ...
  // 2️⃣ Marcar invitación como aceptada (permitido por rules)
